...
def test_bot_create_over_limit_is_rejected(client_as, ic, db, monkeypatch):
    ...
    resp = client_as(ic).post(
        ...
        json={"slack_user_id": "U123", "leave_type": "earned",
              "note": "Holiday", "start_date": start, "end_date": end},
    )
...
def test_bot_create_for_admin_is_allowed_over_limit(client_as, ic, db, monkeypatch):
    from core.config import settings
    monkeypatch.setattr(settings, "INTERNAL_API_KEY", "test-key")
    ...
    resp = client_as(ic).post(
        ...
        headers={"x-internal-key": "test-key"},
        json={"slack_user_id": "U123", "leave_type": "earned",
              "note": "Approved by HR", "start_date": start, "end_date": end},
    )
